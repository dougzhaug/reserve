...
@section('content')
    <div class="box-body">
        <form id="formSearch" class="form-horizontal form-search" method="POST" action="{{url('managers/index')}}">
    ...
    <div class="table-responsive">
        <table id="data-tables" class="table table-striped product-overview" data-url="{{url('managers/index')}}">
            <thead>
                <tr>
...
@push('script')
    <script>
            ...
            return [
                {
                    ...
                    "render": function (data,type,row){
                        ...
                        return `<button type="button" onclick="toggleStatus(this)" data-url="{{url('managers/status')}}`+`/` + row.id + `" data-status="` + row.status + `" class="btn btn-` + btn +` btn-circle"><i class="fa fa-` + i + `"></i> </button>`;
                    }
                },
            ];
        }
        ...
        {
            var html = '';
            html += '<a href="managers/'+data.id+'/edit" class="btn btn-primary btn-xs tables-console tables-edit"><span class="glyphicon glyphicon-edit"></span>编辑</a>';
            html += '<a href="{{url('managers/destroy')}}/'+data.id+'" class="btn btn-danger btn-xs tables-console tables-delete"><span class="glyphicon glyphicon-trash"></span>删除</a>';
            return html;
        }
