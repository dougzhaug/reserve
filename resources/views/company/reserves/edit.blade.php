...
@section('content')
    <h3 class="box-title m-b-0">编辑预约</h3>
    <p class="text-muted m-b-30 font-13"> Use Bootstrap's predefined grid classes for horizontal form </p>
    <form class="form-horizontal" action="{{url('reserves',[$reserve['id']])}}" method="post">
        {{ csrf_field() }}
        {{ method_field('PATCH')}}
        ...
        <div class="form-group {{ $errors->has('name') ? ' has-error' : '' }}">
            ...
            <div class="col-sm-9">
                <input name="name" type="text" value="{{$reserve['name'] or old('name') }}" class="form-control" id="inputName" placeholder="名称">
                @if ($errors->has('name'))
        ...
        <div class="form-group {{ $errors->has('min_ahead_days') ? ' has-error' : '' }}">
            ...
            <div class="col-sm-9">
                <input name="min_ahead_days" type="text" value="{{$reserve['min_ahead_days'] or old('min_ahead_days') }}" class="form-control" id="inputMin" placeholder="可预约的起始天数">
        ...
        <div class="form-group {{ $errors->has('max_ahead_days') ? ' has-error' : '' }}">
            ...
            <div class="col-sm-9">
                <input name="max_ahead_days" type="text" value="{{$reserve['max_ahead_days'] or old('max_ahead_days') }}" class="form-control" id="inputMax" placeholder="可预约的结束天数">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                ...
                {{-- Switchery 开关插件 --}}
                @include('layouts.plugins.Switchery',['name'=>'status','checked'=>$reserve['status']])
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <ul class="nav nav-tabs" role="tablist">
                    <li role="presentation" class="@if ($reserve['type'] == 1) active @endif"><a href="javascript:void(0);" class="type-a" aria-controls="profile" role="tab" data-toggle="tab" aria-expanded="false"><span><i class="fa fa-credit-card"></i></span> 唯一模式</a></li>
                    <li role="presentation" class="@if ($reserve['type'] == 2) active @endif"><a href="javascript:void(0);" class="type-a" aria-controls="home" role="tab" data-toggle="tab" aria-expanded="true"><span><i class="fa fa-cc-paypal text-info"></i></span> 时间间隔模式</a></li>
                    <li role="presentation" class="@if ($reserve['type'] == 3) active @endif"><a href="javascript:void(0);" class="type-a" aria-controls="home" role="tab" data-toggle="tab" aria-expanded="true"><span><i class="fa fa-cc-paypal text-info"></i></span> 排号机制模式</a></li>
                </ul>
                <div class="tab-content">
                    <div role="tabpanel" class="tab-pane @if ($reserve['type'] == 1) active @endif" id="type-only">
                        <div class="col-md-7 col-sm-5">
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    ...
                                    {{-- Radio 单选框插件 --}}
                                    @include('layouts.plugins.Radio',['name'=>'only[status]','style'=>'info','level'=>1,'checked'=>$reserve['config']['only']['status'],'radio'=>[['name'=>'开启','value'=>1],['name'=>'关闭','value'=>0]]])
                                </div>
                            </div>
                        </div>
                    ...
                    <div role="tabpanel" class="tab-pane @if ($reserve['type'] == 2) active @endif" id="type-time-interval">
                        <div class="col-md-7 col-sm-5">
                                ...
                                <tbody>
                                    @foreach($interval as $k=>$v)
                                        <tr class="interval-item">
                                            <td>
                                                <a href="javascript:void(0);" class="editable editable-click editable-empty times" data-type="combodate" data-value="{{$v['start_time']}}" data-template="HH:mm" data-format="HH:mm" data-viewformat="HH:mm" data-title="Start Time" data-original-title="" title="">{{$v['start_time']}}</a>
                                                <input name="interval[start_time][]" type="hidden" value="{{$v['start_time']}}">
                                            </td>
                                            <td>
                                                <a href="javascript:void(0);" class="editable editable-click editable-empty times" data-type="combodate" data-value="{{$v['end_time']}}" data-template="HH:mm" data-format="HH:mm" data-viewformat="HH:mm" data-title="End Time" data-original-title="" title="">{{$v['end_time']}}</a>
                                                <input name="interval[end_time][]" type="hidden" value="{{$v['end_time']}}">
                                            </td>
                                            <td>
                    ...
                    <div role="tabpanel" class="tab-pane @if ($reserve['type'] == 3) active @endif" id="type-queuing">
                        <div class="col-md-7 col-sm-5">
                            <div class="form-group ">
                                <label for="inputNumber" class="col-sm-2 control-label">可预约数</label>
                                <div class="col-sm-9">
                                    <input name="queuing[number]" value="{{$reserve['config']['queuing']['number']}}" type="text" class="form-control" id="inputNumber" placeholder="可预约数量">
                                </div>
                            </div>
...
<script type="text/javascript">
    $(function () {
        //设置初始化时间
        $.cookie('time-interval','{{end($interval)['end_time']}}');
        //初始化x-editable
        XeditableInit();
    })
